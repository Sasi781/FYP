...
class DDI_model(object):
    ...
    def test(self, test_x, test_y, exp_df):
        ...
        ori_test_prediction_predicted_label_df, ori_test_prediction_perf_df, thr = external_validation_v2(self.model, test_x, test_y, exp_df=exp_df, optimal_threshold=self.optimal_threshold, batch_size=1024)
        swi_test_prediction_predicted_label_df, swi_test_prediction_perf_df, thr = external_validation_v2(self.model, switch_x, test_y, exp_df=exp_df, optimal_threshold=self.optimal_threshold, batch_size=1024)
        print('Test set predicted === ')

        merge_predicted_label_df = merge_both_pairs(ori_test_prediction_predicted_label_df, swi_test_prediction_predicted_label_df, thr, 'optimal_thr')
        # print(merged_perf_df)
        # print('AUC: {:.3f}, AUPR: {:.3f}'.format(merged_perf_df.describe().loc['mean']['AUC'], merged_perf_df.describe().loc['mean']['AUPR']))
        return merge_predicted_label_df
